...
def seed_database():
    ...
    if not patient:
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (caregiver_id, "Aarav Sharma", 74, "Assam", "as-IN")
        )
    ...
    for day_offset in range(6, -1, -1):
        ...
        medication_taken = 0 if day_offset == 0 else 1
        hydration_ml = random.randint(1400, 2100)
        ...
        # Model gradual increase in speech pause and cognitive drift over time
        drift_score = round(1.2 + (7 - day_offset) * 0.15 + random.uniform(-0.1, 0.1), 2)
        pause_sec = round(2.0 + (7 - day_offset) * 0.2 + random.uniform(-0.2, 0.3), 2)
        shimmer_pct = round(4.5 + random.uniform(-0.5, 0.8), 2)
        ...
        for hour in range(24):
            ...
            if 16 <= hour <= 20:
                valence = round(random.uniform(-0.6, -0.2), 2)
                arousal = round(random.uniform(0.68, 0.92), 2)
    ...
    conn.commit()
    conn.close()
    print("✅ Successfully seeded 7-day trajectories, agitation heatmaps, adherence records, and game sessions!")
